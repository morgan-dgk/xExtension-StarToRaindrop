  ...
  private $base_url = 'https://api.raindrop.io/v1/oauth/';
	...
	public function jsVarsAction()
	{
		...
		$this->view->stp_vars = json_encode(array(
			...
			'i18n' => array(
				'added_article_to_pocket' => _t('ext.starToRaindropIO.notifications.added_article_to_pocket', '%s'),
				'failed_to_add_article_to_pocket' => _t('ext.starToRaindropIO.notifications.failed_to_add_article_to_pocket', '%s'),
				'ajax_request_failed' => _t('ext.starToRaindropIO.notifications.ajax_request_failed'),
				'article_not_found' => _t('ext.starToRaindropIO.notifications.article_not_found'),
			)
		));
	...
	public function indexAction()
  {
    ...
    $url_redirect = array('c' => 'extension', 'a' => 'configure', 'params' => array('e' => 'StarToRaindropIO'));
		...
		if ($result['status'] == 200) {
			...
			Minz_Request::good(_t('ext.starToRaindropIO.notifications.authorized_success'), $url_redirect);
		} else {
			if ($result['errorCode'] == 158) {
				Minz_Request::bad(_t('ext.starToRaindropIO.notifications.authorized_aborted'), $url_redirect);
			} else {
				Minz_Request::bad(_t('ext.starToRaindropIO.notifications.authorized_failed', $result['errorCode']), $url_redirect);
			}
		}
	}
